#tag Class
Protected Class ConsoleObject
	#tag Method, Flags = &h1
		Protected Sub AddCTRLHandler(HandlerFunction As CTRL_Event_HandlerRoutine)
		  Dim err As Console.Win32Exception
		  If Not SetConsoleCtrlHandler(HandlerFunction, True) Then
		    err = New Console.Win32Exception(GetLastError)
		    Raise err
		  End If
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function ClearScreen() As Boolean
		  ...
		  #If Not TargetHasGUI And TargetWin32 Then  //Windows Console Applications only
		    Dim cord As COORD = Buffer.Dimensions
	...
	#tag Method, Flags = &h0
		Sub Constructor()
		  AddCTRLHandler(AddressOf DefaultCTRLHandler)
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h21
		Private Function DefaultCTRLHandler(EventType As Integer) As Boolean
		  Return ControlEvent(EventType)
		End Function
	#tag EndMethod
		...
		Function GetChar(x As Integer, y As Integer) As String
		  ...
		  #If Not TargetHasGUI And TargetWin32 Then  //Windows Console Applications only
		    ...
		    If ReadConsoleOutputCharacter(StdOutHandle, mb, mb.Size, cords, p) Then
		      Return mb.CString(0)
		    Else
		      Raise New Console.Win32Exception(GetLastError)
		    End If
	...
	#tag Method, Flags = &h1
		Protected Sub RemoveCTRLHandler(HandlerFunction As CTRL_Event_HandlerRoutine)
		  Dim err As Console.Win32Exception
		  If Not SetConsoleCtrlHandler(HandlerFunction, False) Then
		    err = New Console.Win32Exception(GetLastError)
		    Raise err
		  End If
		End Sub
	...
	#tag Method, Flags = &h0
		Sub SendControlEvent(EventType As Integer)
		  'Sends a Control + Key signal. EventType may be CTRL_C_EVENT (CTRL+C) or CTRL_BREAK_EVENT (CTRL+Break). Set this
		  'to Nil to remove the console event handler.
		  
		  #If Not TargetHasGUI And TargetWin32 Then  //Windows Console Applications only
		    Call GenerateConsoleCtrlEvent(EventType, 0)
		  #endif
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub WrappingPrint(Msg As String, LinePrefix As String = "")
		  ...
		  #If TargetWin32 And Not TargetHasGUI Then
		    Dim bufferwidth As Integer = Me.Buffer.Dimensions.X
	...
	#tag Hook, Flags = &h0
		Event ControlEvent(EventType As Integer) As Boolean
	#tag EndHook
		...
		#tag Setter
			Set
			  #If Not TargetHasGUI And TargetWin32 Then  //Windows Console Applications only
			    ...
			    cord.X = 0
			    cord.Y = 0
			    Call FillConsoleOutputAttribute(StdOutHandle, Console.ShiftColor(value), charCount, cord, charCount)
			  #Else
			    #pragma Unused NewColors
			  #EndIf
			End Set
	...
	#tag ComputedProperty, Flags = &h0
		...
		#tag Setter
			Set
			  ...
			  #If Not TargetHasGUI And TargetWin32 Then  //Windows Console Applications only
			    Dim cord As COORD
			    Dim winSize As SMALL_RECT = Buffer.WindowRect
	...
	#tag ComputedProperty, Flags = &h0
		#tag Getter
			...
			End Get
		#tag EndGetter
		ConsoleOriginalTitle As String
	#tag EndComputedProperty
